// Gemini REST API의 Schema 타입은 대문자 문자열("OBJECT"/"ARRAY"/"STRING"/"BOOLEAN")을 그대로 쓴다.
...
- link 필드는 검색 결과에서 실제로 열어본 페이지의 URL을 정확히 그대로 복사한다. 슬러그, 날짜, 경로를 기억에 의존해 추측하거나 재구성하지 않는다.
- link는 반드시 개별 기사 페이지(article permalink)여야 한다. 여러 기사를 모아놓은 허브/카테고리/태그/섹션 목록 페이지(예: "/hub/", "/topic/", "/tag/" 등)는 link로 쓰지 않는다. 개별 기사 페이지를 찾지 못했다면 그 기사는 후보에서 제외한다.
- 영어(해외) 기사를 우선 검색한다. 동일한 수준의 기사가 있다면 한국어 기사보다 영어 기사를 우선한다.
- 날짜는 최근 3~4일 이내 기사를 최우선으로 찾는다. 3~4일 이내 기사가 부족하면 최근 7일까지 범위를 넓혀서 채우고, 그래도 부족하면 한국어 기사나 7일이 넘은 기사로 채운다(빈 결과보다 채우는 것을 우선한다).
...
export const TREND_CONNECTION_SCHEMA = {
  type: 'OBJECT',
  properties: {
    related_articles: { type: 'ARRAY', items: { type: 'STRING' }, description: '연결되는 이전 기사 제목 (겹치는 게 없으면 빈 배열)' },
    related_technologies: { type: 'ARRAY', items: { type: 'STRING' } },
    related_companies: { type: 'ARRAY', items: { type: 'STRING' } },
    industry_flow: { type: 'STRING', description: '밸류체인 위치 및 산업 흐름 설명' },
    further_study: { type: 'ARRAY', items: { type: 'STRING' }, description: '개념명: 왜 관련 있는지 한 줄' },
  },
